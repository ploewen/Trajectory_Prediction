...
def main():
    ...
    project_dir = (
        script_dir.parent
        if script_dir.name in ("scripts", "training", "utils")
        else script_dir
    )
    ...
    if all_good:
        ...
        print("\nNext steps:")
        print(
            "  1. python training/train.py --run-stages   # Train all configured stages"
        )
        print(
            "  2. python training/eval.py experiments/cars_velocity/best_model.pt  # Evaluate a checkpoint"
        )
        print(
            "  3. python training/example_inference.py --checkpoint experiments/cars_velocity/best_model.pt  # Demo inference"
        )
        return 0
    else:
