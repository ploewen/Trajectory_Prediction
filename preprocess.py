...
Saves as PyTorch tensors: train_x.pt, test_x.pt (history) and train_y.pt, test_y.pt (future)
"""
...
def process_scenes():
    ...
    if len(all_histories) < TRAIN_SIZE:
        print(
            f"[ERROR] Need at least {TRAIN_SIZE} trajectories for the training set, "
            f"but found {len(all_histories)}."
        )
        return

    all_x = torch.tensor(np.array(all_histories), dtype=torch.float32)
    all_y = torch.tensor(np.array(all_futures), dtype=torch.float32)

    # Shuffle once, then take a fixed-size training set.
    generator = torch.Generator().manual_seed(RANDOM_SEED)
    indices = torch.randperm(all_x.shape[0], generator=generator)
    ...
    print(f"[INFO] test_x shape: {test_x.shape}")
    print(f"[INFO] test_y shape: {test_y.shape}")

    torch.save(train_x, f"{OUTPUT_DIR}/train_x.pt")
    torch.save(train_y, f"{OUTPUT_DIR}/train_y.pt")
    torch.save(test_x, f"{OUTPUT_DIR}/test_x.pt")
    torch.save(test_y, f"{OUTPUT_DIR}/test_y.pt")
