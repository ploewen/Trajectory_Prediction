...
class TransformerTrajectoryPredictor(nn.Module):
    ...
    def __init__(
        ...
        self.positional_encoding = PositionalEncoding(
            d_model=d_model, max_len=5000, dropout=dropout
        )
...
class LSTMGRUPredictor(nn.Module):
    """
    Predicts 12 future (x, y) points from 4 history (x, y) points.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for _ in range(self.future_frames):
            ...
            next_point = self.output_layer(decoder_output.squeeze(1))
...
if __name__ == "__main__":
    ...
    model = create_model(
        num_input_frames=4, num_output_frames=12, num_features=2, device=device
    )
    ...
    model_with_vel = create_model(
        num_input_frames=4, num_output_frames=12, num_features=4, device=device
    )
    ...
    model_20frames = create_model(
        num_input_frames=4, num_output_frames=20, num_features=2, device=device
    )
